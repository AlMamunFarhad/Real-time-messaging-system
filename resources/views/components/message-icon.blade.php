...
@if ($isAdminDashboard)
    <div x-data="window.messageCounterBadge({
        ...
        <a href="{{ route('admin.messages') }}"
            class="relative inline-flex p-2 text-gray-600 transition-colors duration-200 hover:text-blue-600">
...
@elseif (\App\Models\Admin::first())
    <div x-data="window.userMessagePanel({
        unreadCount: {{ $unread }},
        userId: {{ $userId ?? 0 }},
        ...
        <button @click="openChat()"
            class="relative p-2 text-gray-600 transition-colors duration-200 hover:text-blue-600">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24"
                stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
        ...
        <div x-show="open" x-transition.opacity class="fixed inset-0 z-[70] bg-slate-950/45 backdrop-blur-sm"
            style="display:none;">
            <div class="flex min-h-screen items-center justify-center p-4">
                <div @click.outside="closeChat()"
                    class="flex h-[85vh] w-full max-w-5xl flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_24px_80px_-28px_rgba(15,23,42,0.45)]">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <div
                                    ...
                                    <span class="text-base font-semibold"
                                        x-text="otherParticipantName ? otherParticipantName.charAt(0).toUpperCase() : 'A'"></span>
                                </div>
                                <div>
                                    <h3 class="text-base font-semibold text-slate-900" x-text="otherParticipantName">
                                    </h3>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <button type="button" @click="closeChat()"
                                    class="rounded-2xl border border-slate-200 px-3 py-2 text-sm text-slate-600 transition hover:bg-slate-50">Close</button>
                            </div>
                        </div>
                    </div>

                    <div id="user-chat-box"
                        class="flex-1 overflow-y-auto bg-[radial-gradient(circle_at_top,#f8fafc_0%,#ffffff_42%,#f8fafc_100%)] px-6 py-6">
                        ...
                        <div id="user-chat-empty-note" x-show="!loadingChat"
                            class="rounded-3xl border border-dashed border-slate-300 bg-white/80 px-6 py-10 text-center text-slate-500">
                            <p class="text-sm font-medium">Chat load ??? ?????</p>
                        </div>
                    </div>

                    <div class="border-t border-slate-200 bg-white px-6 py-4">
                        <div id="user-file-preview" style="display:none;"
                        ...
                        <div class="flex items-end gap-3">
                            <input type="file" id="user-file-input" class="hidden"
                                @change="handleFileSelect($event)">
                            <button type="button" @click="document.getElementById('user-file-input').click()"
                            ...
                            <input type="text" id="user-message-input" @keydown.enter.prevent="sendMessage()"
...
@if ($isAdminDashboard || \App\Models\Admin::first())
    ...
    <script>
        ...
        window.buildUserAttachmentHtml = window.buildUserAttachmentHtml || function(fileUrl, fileName, isMe) {
            if (!fileUrl) return '';
        ...
        window.userMessagePanel = window.userMessagePanel || function(config) {
            return {
                open: false,
                unreadCount: config.unreadCount || 0,
                loadingChat: false,
                ...
                renderMessage(message) {
                    ...
                    content += window.buildUserAttachmentHtml(fileUrl, fileName, isMe);
                ...
                startPolling() {
                    ...
                    this.pollTimer = setInterval(async () => {
                        ...
                        try {
                            const response = await axios.get('/messages/' + this.activeConversationId + '?t=' +
                                Date.now());
                            const messages = response.data.messages || [];
                ...
                async checkOnlineStatus() {
                    ...
                    try {
                        const response = await axios.get('/online-status/' + this.otherParticipantId + '/' + this
