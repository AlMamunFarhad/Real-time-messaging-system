<div x-data="messagingDashboard({
    ...
    <div x-show="isWorkspaceVisible" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-[0_24px_80px_-28px_rgba(15,23,42,0.35)]">
        <div class="grid h-[78vh] lg:grid-cols-[320px_minmax(0,1fr)]">
            <aside class="border-b border-slate-200 bg-slate-50 lg:border-b-0 lg:border-r">
                <div class="border-b border-slate-200 bg-white px-5 py-5">
                    ...
                    <div class="mt-6 flex items-center gap-1 rounded-[20px] bg-slate-100 p-1.5">
                        <button type="button" 
                            ...
                            :class="activeTab === 'contacts' ? 'bg-white text-slate-900 shadow-[0_2px_8px_-2px_rgba(15,23,42,0.12)]' : 'text-slate-500 hover:text-slate-900 hover:bg-white/50'"
                            class="flex flex-1 items-center justify-center gap-1.5 rounded-[14px] py-2.5 text-[11px] font-bold transition-all duration-200">
                ...
                <div class="px-5 py-5 overflow-hidden">
                    ...
                    <div x-show="activeTab === 'direct'" class="flex flex-col h-[52vh]" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 -translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-4">Recent Conversations</p>
                        
                        <div class="flex-1 space-y-2.5 overflow-y-auto pr-1">
                            <template x-if="!directConversations.length">
                                <div class="rounded-3xl border border-dashed border-slate-200 bg-white px-4 py-12 text-center">
                                    <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-50 text-slate-300 mb-3">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                        </svg>
                                    </div>
                                    <p class="text-xs font-medium text-slate-400 leading-relaxed">No chats found.<br>Go to Contacts to start one.</p>
                                </div>
                            </template>
                            <template x-for="conversation in directConversations" :key="conversation.id">
                                <button type="button" @click="selectConversation(conversation)" class="group block w-full rounded-2xl border px-4 py-3.5 text-left transition duration-200" :class="activeConversationId === conversation.id ? 'border-slate-900 bg-slate-900 text-white' : 'border-transparent bg-white text-slate-800 hover:border-slate-200 hover:bg-slate-50'">
                                    <div class="relative min-w-0 flex-1">
                                        <div class="flex items-center gap-2">
                                            <template x-if="conversation.is_online">
                                                <span class="h-2 w-2 shrink-0 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></span>
                                            </template>
                                            <div class="truncate text-sm font-bold" x-text="conversation.title"></div>
                                        </div>
                                        <div class="mt-0.5 truncate text-[11px]" :class="activeConversationId === conversation.id ? 'text-slate-400' : 'text-slate-500'" x-text="conversation.last_message?.body || 'No message yet'"></div>
                                    </div>
                                    <template x-if="Number(conversation.unread_count || 0) > 0">
                                        <span class="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-rose-500 px-1.5 text-[10px] font-black text-white" x-text="conversation.unread_count"></span>
                                    </template>
                                </button>
                            </template>
                        </div>
                    ...
                    <div x-show="activeTab === 'groups'" class="flex flex-col h-[52vh]" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                        <div class="mb-5 flex items-center justify-between">
                            <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400">Team Groups</p>
                            <button type="button" @click="openCreateGroup()" class="inline-flex h-8 items-center rounded-xl bg-slate-900 px-3 text-[10px] font-black tracking-wider text-white transition hover:bg-slate-800">
                                + NEW
                            </button>
                        </div>
                        ...
                        <div class="flex-1 space-y-2.5 overflow-y-auto pr-1">
                            <template x-if="!groupConversations.length">
                                <div class="rounded-3xl border border-dashed border-slate-200 bg-white px-4 py-12 text-center text-xs text-slate-400">
                                    Collaborate with your team<br>in group chats.
                                </div>
                            </template>
                            <template x-for="conversation in groupConversations" :key="conversation.id">
                                <button type="button" @click="selectConversation(conversation)" class="block w-full rounded-2xl border px-4 py-3.5 text-left transition duration-200" :class="activeConversationId === conversation.id ? 'border-slate-900 bg-slate-900 text-white' : 'border-transparent bg-white text-slate-800 hover:border-slate-200 hover:bg-slate-50'">
                                    <div class="flex items-center justify-between gap-3">
                                        <div class="min-w-0">
                                            <div class="truncate text-sm font-bold" x-text="conversation.title"></div>
                                            <div class="mt-0.5 truncate text-[11px]" :class="activeConversationId === conversation.id ? 'text-slate-400' : 'text-slate-500'" x-text="conversation.last_message?.body || 'Start a discussion'"></div>
                                        </div>
                                        <template x-if="Number(conversation.unread_count || 0) > 0">
                                            <span class="inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-rose-500 px-1.5 text-[10px] font-black text-white" x-text="conversation.unread_count"></span>
                                        </template>
                    ...
                    <div x-show="activeTab === 'contacts'" class="flex flex-col h-[52vh]" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-2" x-transition:enter-end="opacity-100 translate-x-0">
                        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-400 mb-4">Find Members</p>
                        
                        <div class="relative">
                            <input x-ref="directSearchInput" x-model="directSearch" @input.debounce.250ms="loadDirectCandidates()" type="text" placeholder="Type a name..." class="w-full rounded-2xl border border-slate-100 bg-white px-4 py-3 text-sm text-slate-700 outline-none focus:border-slate-300 focus:ring-4 focus:ring-slate-100 shadow-sm transition-all">
                        ...
                        <div class="flex-1 mt-5 space-y-2 overflow-y-auto pr-1">
                            <template x-if="!directCandidates.length && directSearch">
                                <div class="px-4 py-12 text-center text-xs text-slate-400">User not found. Try another name.</div>
                            </template>
                            <template x-for="item in directCandidates" :key="item.type + '-' + item.id">
                                <button type="button" @click="startDirectChat(item)" class="group flex w-full items-center justify-between rounded-2xl border border-transparent bg-white px-4 py-3.5 text-left transition duration-200 hover:border-slate-200 hover:bg-slate-50">
                                    <div class="min-w-0">
                                        <div class="flex items-center gap-2">
                                            <template x-if="item.is_online">
                                                <span class="h-2 w-2 shrink-0 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></span>
                                            </template>
                                            <div class="truncate text-sm font-bold text-slate-900" x-text="item.name"></div>
                                        </div>
                                        <div class="mt-0.5 truncate text-[10px] uppercase font-bold tracking-wider text-slate-400" x-text="item.subtitle"></div>
                                    </div>
                                    <div class="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-50 text-slate-400 group-hover:bg-slate-900 group-hover:text-white transition-colors">
            ...
            <section class="flex h-[78vh] flex-col bg-[radial-gradient(circle_at_top,#f8fafc_0%,#ffffff_42%,#f8fafc_100%)]">
                <template x-if="activeConversation">
                    <div class="flex flex-1 flex-col overflow-hidden">
                            ...
                            <div class="flex flex-wrap items-start justify-between gap-4">
                                <div>
                                    <div class="flex items-center gap-3">
                                        <div class="relative">
                                            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-amber-300 text-slate-900 shadow-sm">
                                                <span class="text-base font-semibold" x-text="initialFor(activeConversation.title)"></span>
                                            </div>
                                            <template x-if="activeConversation.is_online">
                                                <span class="absolute -right-0.5 -bottom-0.5 block h-3.5 w-3.5 rounded-full border-2 border-white bg-green-500 shadow-sm"></span>
                                            </template>
                                        </div>
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <h3 class="text-lg font-semibold text-slate-900" x-text="activeConversation.title"></h3>
                                                <template x-if="activeConversation.is_online">
                                                    <span class="text-[10px] font-bold uppercase tracking-wider text-green-500 bg-green-50 px-2 py-0.5 rounded-full">Online</span>
                                                </template>
                                            </div>
                                            <p class="text-sm text-slate-500" x-text="activeConversation.is_group ? ((groupDetails.members?.length || activeConversation.members_count || 0) + ' members in this group') : (activeConversation.is_online ? 'Active now' : 'Personal chat')"></p>
                                        </div>
                                    </div>
                                ...
                                <div class="flex flex-wrap items-center gap-2">
                                    <button type="button" @click="closeWorkspace()" class="rounded-full border border-slate-200 bg-white p-2 text-slate-600 transition hover:bg-slate-50 hover:text-slate-900" title="Close Workspace">
                        ...
                        <div id="messages-panel" class="flex-1 overflow-y-auto px-6 py-6 scroll-smooth">
